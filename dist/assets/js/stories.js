pdfjsLib.GlobalWorkerOptions.workerSrc =
  ...
  function applyTranslations(lang) {
    ...
    const pt = document.querySelector('[data-key="pageTitle"]'); if(pt) pt.textContent = t.pageTitle;
    const hm = document.querySelector('[data-key="home"]'); if(hm) hm.textContent = t.home;
    const st = document.querySelectorAll('[data-key="stories"]'); st.forEach(el => el.textContent = t.stories);
    ...
    const poolsText=document.querySelector("#slide-1 .pools-text"); if(poolsText) poolsText.innerHTML=t.s1pools;
  ...
  async function initNileEvoFlip(lang){
    ...
    pageFlip.on("flip", (e) => {
      ...
      showSlide(isLast || isCov ? "cover" : getSlideForPage(logicalPage));
